...
def train_net(param, model, train_data, valid_data, plot=False,device='cuda'):
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=3, T_mult=2, eta_min=8e-6, last_epoch=-1)
    criterion = nn.CrossEntropyLoss(reduction='mean', ignore_index=0).to(device)
    # DiceLoss_fn=DiceLoss()
    # SoftCrossEntropy_fn=CrossEntropyLoss()
    # criterion = L.JointLoss(first=DiceLoss_fn, second=SoftCrossEntropy_fn,
    #                           first_weight=0.5, second_weight=0.5).cuda()
    logger = inial_logger(os.path.join(save_log_dir, time.strftime("%m-%d %H_%M_%S", time.localtime()) +'_'+model_name+ '.log'))

    # 主循环
    train_loss_total_epochs, valid_loss_total_epochs, epoch_lr, miou_total_epochs = [], [], [], []
    ...
    for epoch in range(epoch_start, epochs):
        ...
        miou_total_epochs.append(float(mean_iu))
    ...
    if plot:
        ...
        #loss
        plt.figure(1)  
        plt.plot(x, smooth(valid_loss_total_epochs, 0.6), label="val loss")
        ...
        #iou
        plt.figure(2)  
        plt.plot(x, smooth(miou_total_epochs, 0.6), label="val miou")
        plt.xlabel('Epoch', fontsize=15)
        plt.ylabel('Val miou', fontsize=15)
        plt.grid(True)
        plt.show()
    ...
    #     plt.legend(loc='upper right', fontsize=15)
    #     plt.show()
    #     plt.savefig("performace_without_pretrain.png")
